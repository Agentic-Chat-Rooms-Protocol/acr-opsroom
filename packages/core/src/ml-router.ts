...
import { OpsIncident, OpsSeverity, TelemetryMetric, AgentSquad, AgentProfile } from './types.js';
...
export class MLStatisticalDetector {
   ...
   */
  public static computeBaseline(samples: number[]): StatisticalSummary {
    if (samples.length === 0) {
      return {
        count: 0,
        mean: 0,
...
export class MLIncidentRouter {
  private static predefinedSquads: SquadVector[] = [
    {
   ...
   */
  public static routeIncidentToSquad(
    title: string,
    description: string,
    anomalies: AnomalyReport[]
  ): { squadId: string; squadName: string; similarityScore: number; recommendedSeverity: OpsSeverity } {
    ...
    const severityHierarchy: Record<OpsSeverity, number> = { P0: 4, P1: 3, P2: 2, P3: 1 };
    let maxSeverity: OpsSeverity = 'P3';
   ...
   */
  public static createDefaultSquad(squadId: string): AgentSquad {
