...
describe('MLIncidentRouter & Squad Dispatch', () => {
    test('routes database replication incident to SRE Reliability Squad', () => {
        ...
        const routed = MLIncidentRouter.routeIncidentToSquad('Postgres Read Replica Out of Sync', 'High replication lag detected on follower db-2 with split-brain risk', anomalies);
    ...
    test('routes unauthorized egress incident to SecOps Zero-Trust Squad', () => {
        ...
        const routed = MLIncidentRouter.routeIncidentToSquad('Suspicious Token Exfiltration Detected', 'Unauthorized egress traffic to unknown IP address with token invalidation', anomalies);
        assert.equal(routed.squadId, 'squad-secops-defense');
        assert.ok(routed.squadName.includes('SecOps'));
    });
});
//# sourceMappingURL=ml-router.test.js.map