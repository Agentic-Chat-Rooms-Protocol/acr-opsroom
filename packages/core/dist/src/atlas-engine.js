...
export class Atlas2Engine {
    static MAX_DELIBERATION_TURNS = 10;
    /**
     * Initialize a new OpsIncident with grounded state machine.
     */
    static createIncident(title, description, squad) {
     ...
     */
    static transitionPhase(incident, targetStatus) {
    ...
    static mapStatusToPhase(status) {
        switch (status) {
     ...
     */
    static addDeliberationTurn(incident, agent, reasoning, proposedHypothesis, proposedActions = [], confidence = 0.85) {
        // Loop-cycling guard
     ...
     */
    static compileExecutionPlan(incident, targetGoal) {
        const steps = [];
        let stepNumber = 1;
...
//# sourceMappingURL=atlas-engine.js.map