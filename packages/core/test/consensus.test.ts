...
import { ConsensusBallot } from '../src/types.js';
...
describe('ByzantineConsensusEngine', () => {
  ...
  test('detects Byzantine ballot anomalies (tampered DID or negative confidence)', () => {
    ...
    const corruptedBallot: ConsensusBallot = {
      ...validBallot,
  ...
  test('detects signature forgery and payload tampering', () => {
    ...
    const tamperedBallot: ConsensusBallot = {
      ...validBallot,
      decision: 'approve',
    };
    ...
    const forgedBallot: ConsensusBallot = {
      ...validBallot,
  ...
  test('detects Sybil duplicate and equivocation attacks in quorum evaluation', () => {
    ...
    const ballots: ConsensusBallot[] = [b0, b1, b2Approve, b2Reject];
